...
export default function Gallery({ eventKey }: { eventKey: string }) {
  ...
  return <main className={`page ${isSelecting ? "selecting" : ""}`}>
    ...
    {showInfo && <div className="info-overlay" role="dialog" aria-modal="true" aria-label="ご案内" onClick={() => setShowInfo(false)}><section className="info-dialog" onClick={(event) => event.stopPropagation()}><button className="info-close" aria-label="閉じる" onClick={() => setShowInfo(false)}>×</button><p className="eyebrow">Information</p><h2>お問い合わせ</h2><p>お気づきやお問い合わせは以下までお願いします。</p><a href="mailto:sota304560@gmail.com">sota304560@gmail.com</a><p className="github-link"><a href="https://github.com/uenooooo/wedPhoto" target="_blank" rel="noreferrer">GitHub</a></p></section></div>}
  </main>;
}
