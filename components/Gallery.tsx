...
export default function Gallery({ eventKey }: { eventKey: string }) {
  ...
  const downloadSelected = () => {
    ...
    if (selectedItems.length >= 20 || selectedItems.some((item) => item.type === "video")) void requestArchive(selectedItems.map((item) => item.key));
  ...
  return <main className={`page ${isSelecting ? "selecting" : ""}`}>
    ...
    {isSelecting && <div className="selection-actionbar"><button className="button" disabled={selectedCount === 0} onClick={downloadSelected}>{selectedCount >= 20 || items.some((item) => selected.includes(item.key) && item.type === "video") ? "ZIPを作成してダウンロード" : "選択した写真をダウンロード"}</button></div>}
    ...
    {showInfo && <div className="info-overlay" role="dialog" aria-modal="true" aria-label="ご案内" onClick={() => setShowInfo(false)}><section className="info-dialog" onClick={(event) => event.stopPropagation()}><button className="info-close" aria-label="閉じる" onClick={() => setShowInfo(false)}>×</button><p className="eyebrow">Information</p><h2>お問い合わせ</h2><p>お気づきやお問い合わせは以下までお願いします。</p><a href="mailto:sota304560@gmail.com">sota304560@gmail.com</a><div className="github-link"><span>ソースコード</span><a href="https://github.com/uenooooo/wedPhoto" target="_blank" rel="noreferrer">GitHubで見る ↗</a></div></section></div>}
  </main>;
}
