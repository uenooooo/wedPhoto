...
export default function Gallery({ eventKey }: { eventKey: string }) {
  ...
  return <main className={`page ${isSelecting ? "selecting" : ""}`}>
    ...
    {archiveStatus === "ZIPを準備中です…" && <div className="archive-overlay" role="status" aria-live="polite"><div className="archive-dialog"><span className="archive-spinner" /><strong>ZIPを準備中です</strong><p>準備ができ次第、自動でダウンロードします。<br />この画面を閉じずにお待ちください。</p></div></div>}
