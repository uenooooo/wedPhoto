...
export default function Gallery({ eventKey }: { eventKey: string }) {
  ...
  return <main className={`page ${isSelecting ? "selecting" : ""}`}>
    ...
    {isSelecting && <div className="selection-actionbar"><button className="button" disabled={selectedCount === 0 || archiveStatus === "ZIPを準備中です…"} onClick={downloadSelected}>{archiveStatus === "ZIPを準備中です…" ? "ZIPを準備中…" : selectedCount >= 2 ? "ZIPを作成してダウンロード" : "ダウンロード"}</button></div>}
    {archiveStatus === "ZIPを準備中です…" && <div className="archive-overlay" role="status" aria-live="polite"><div className="archive-dialog"><span className="archive-spinner" /><strong>ZIPを準備中です</strong><p>通常は30秒〜1分です。<br />動画や大量の写真は数分かかる場合があります。</p><small>準備ができ次第、自動でダウンロードします。<br />この画面を閉じずにお待ちください。</small></div></div>}
