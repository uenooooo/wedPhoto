...
export default function Gallery({ eventKey }: { eventKey: string }) {
  ...
  return <main className={`page ${isSelecting ? "selecting" : ""}`}>
    ...
    <section className="section gallery-section">
      {isSelecting ? <div className="selection-toolbar"><button onClick={finishSelecting}>キャンセル</button><strong>{selectedCount}件を選択</strong><button onClick={selectAll}>{selectedCount === visibleItems.length ? "選択解除" : "すべて選択"}</button></div> : <div className="toolbar"><div><p className="eyebrow">Memories</p><h2>ギャラリー</h2></div><button className="button subtle" onClick={() => setIsSelecting(true)}>選択</button></div>}
