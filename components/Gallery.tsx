...
export default function Gallery({ eventKey }: { eventKey: string }) {
  ...
  return <main className={`page ${isSelecting ? "selecting" : ""}`}>
    ...
    {isSelecting && <div className="selection-actionbar"><button className="button" disabled={selectedCount === 0} onClick={downloadSelected}>{selectedCount >= 2 ? "ZIPを作成してダウンロード" : "ダウンロード"}</button></div>}
