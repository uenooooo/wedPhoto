...
export const handler = async (event) => {
  for (const record of event.Records ?? []) {
    ...
    if (type.startsWith("video/")) {
      ...
      await mediaConvert.send(new CreateJobCommand({ Role: process.env.MEDIACONVERT_ROLE_ARN, Settings: { Inputs: [{ FileInput: `s3://${bucket}/${key}` }], OutputGroups: [{ OutputGroupSettings: { Type: "FILE_GROUP_SETTINGS", FileGroupSettings: { Destination: `s3://${bucket}/ready/` } }, Outputs: [{ ContainerSettings: { Container: "MP4" }, VideoDescription: { CodecSettings: { Codec: "H_264" } }, AudioDescriptions: [{ CodecSettings: { Codec: "AAC" } }] }] }] } }));
    }
  }
};
